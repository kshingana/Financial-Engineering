...
"if the schema is not not present then only it will be added else it will start directly start the fetching"
def start():
    create_schemas()
    while True:
        start_fetching()
...
def fetch_oi(dajs, expiry_dt, symbol):
    " this is simply for 'CE' in [records][data] is present, it will return the Call option data"
